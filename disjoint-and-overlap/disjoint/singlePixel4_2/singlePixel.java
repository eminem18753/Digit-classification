...
public class singlePixel
{
	...
	public singlePixel(String trainingFilePath,String testFilePath)
	{
		...
		countFeature=new int[indexNumber][rowNumber/rowSize*colNumber/colSize][(int)Math.pow(2,rowSize*colSize)];
		for(int i=0;i<indexNumber;i++)
		{
			for(int j=0;j<rowNumber/rowSize*colNumber/colSize;j++)
			{
		...
		probability=new double[indexNumber][rowNumber/rowSize][colNumber/colSize][rowSize][colSize];
		for(int i=0;i<indexNumber;i++)
		{
			for(int j=0;j<rowNumber/rowSize;j++)
			{
				for(int k=0;k<colNumber/colSize;k++)
				{
		...
		probabilityNothing=new double[indexNumber][rowNumber/rowSize][colNumber/colSize][rowSize][colSize];
		for(int i=0;i<indexNumber;i++)
		{
			for(int j=0;j<rowNumber/rowSize;j++)
			{
	...
	public void singlePixelAlgorithm()
	{
		...
		for(int i=0;i<dataset.size();i++)
		{
			...
			{
				for(int k=0;k<dataset.get(i).get(j).length()/rowSize/colSize;k++)
				{
					quotient=k/16;
					remnant=k%16;
					for(int m=0;m<rowSize;m++)
					{
						for(int n=0;n<colSize;n++)
						{
							...
							//System.out.print(" ");
							if(dataset.get(i).get(j).charAt(quotient*128+remnant*2+m+n*32)=='1')
							{
			...
			for(int k=0;k<rowNumber/rowSize;k++)
			{
				for(int m=0;m<colNumber/colSize;m++)
				{
					for(int n=0;n<rowSize;n++)
					{
						...
						{
							probability[i][k][m][n][o]=countFeature[i][k*(colNumber/colSize)+m][n*colSize+o];
		...
		for(int i=0;i<testSet.size();i++)
		{
			...
			for(int j=0;j<10;j++)
			{
				...
				for(int k=0;k<rowNumber/rowSize;k++)
				{
					for(int m=0;m<colNumber/colSize;m++)
					{
						quotient=(k*colNumber/colSize+m)/16;
						remnant=(k*colNumber/colSize+m)%16;
						for(int n=0;n<rowSize;n++)
						{
							for(int o=0;o<colSize;o++)
							{
								if(testSet.get(i).charAt(quotient*128+remnant*2+n+o*32)=='1')
								{
