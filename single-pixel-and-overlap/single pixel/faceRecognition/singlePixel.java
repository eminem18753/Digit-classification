...
public class singlePixel
{
	...
	public void singlePixelAlgorithm()
	{
		...
		for(int i=0;i<dataset.size();i++)
		{
			...
			{
				if(dataset.get(i).charAt(j)=='#')
				{
					if(trainLabel.get(i)==0)
					{
						countFeature[0][j]=countFeature[0][j]+1;
					}
					else
					{
						countFeature[1][j]=countFeature[1][j]+1;
					}
				}
			}
		}
		...
		for(int i=0;i<2;i++)
		{
			for(int k=0;k<rowNumber;k++)
			{
				...
				{
					probability[i][k][m]=countFeature[i][k*60+m];
				}
			}
		}
		...
		for(int i=0;i<indexNumber;i++)
		{
			for(int j=0;j<rowNumber;j++)
			{
				for(int k=0;k<colNumber;k++)
				{
					probability[i][j][k]=(double)probability[i][j][k]/(double)dataset.size();
		...
		for(int i=0;i<indexNumber;i++)
		{
			for(int j=0;j<rowNumber;j++)
			{
				for(int k=0;k<colNumber;k++)
				{
					probability[i][j][k]=(probability[i][j][k]+smoothingFactor)/(dataset.size()+smoothingFactor*2);
					//System.out.println(probability[i][j][k]);
				}
			}
		}
		...
		for(int i=0;i<testSet.size();i++)
		{
			...
			for(int j=0;j<2;j++)
			{
				...
				for(int k=0;k<69;k++)
				{
					for(int m=0;m<60;m++)
					{
						if(testSet.get(i).charAt(k*60+m)=='#')
						{
